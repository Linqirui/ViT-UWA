...
from .uwa_modules import EdgeAwareModule, VCIM, deform_inputs

_logger = logging.getLogger(__name__)


@BACKBONES.register_module()
class UniPerceiverUWA(UnifiedBertEncoder):
    def __init__(self, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4,
                 deform_num_heads=6, init_values=0., with_cmlp=True, cmlp_ratio=0.25,
        ...
        self.eam = EdgeAwareModule(inplanes=conv_inplane, embed_dim=embed_dim)
        self.interactions = nn.Sequential(*[
            VCIM(dim=embed_dim, num_heads=deform_num_heads, n_points=n_points,
                             ...
                             extra_VCIM_da2=True if i == len(interaction_indexes) - 1 else False)
        ...
        self.eam.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        inp = x
        hfc_tune = self.fft(inp, 0.25)

        # EAM forward
        c1, c2, c3, c4 = self.eam(x)
        c2, c3, c4 = self._add_level_embed(c2, c3, c4)
        c = torch.cat([c2, c3, c4], dim=1)

        # Patch Embedding forward
        x, H, W = self.visual_embed(x)
        I_hfc, _, _ = self.visual_embed(hfc_tune)
        x = x + I_hfc
        bs, n, dim = x.shape
